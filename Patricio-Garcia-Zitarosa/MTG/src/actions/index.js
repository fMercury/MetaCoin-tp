...
export function fetchCards(forContract) {
    return (dispatch, getState) => {
        ...
        web3.eth.getAccounts((err, accounts) => {
            cardsContract.tokensOf(forContract ? cardsContract.address : accounts[0]).then(cards => {
                //dont do this in production :(
                Promise.all(cards.map(card => cardsContract.getIpfsHash(card))).then(values => {
                    let i = 0;
                    dispatch({
...
export function mintCard(payload) {
    return (dispatch, getState) => {
        ...
        web3.eth.getAccounts((err, accounts) => {
            cardsContract
                .mint(payload.hash || '', payload.price || web3.toWei(0.001, 'ether'), {
                    from: accounts[0]
                })
                .then(results => {
                    dispatch({
...
export function buyCard(payload) {
    return (dispatch, getState) => {
        ...
        web3.eth.getAccounts((err, accounts) => {
            cardsContract
                .buyCard(Number(payload.id), {
                    from: accounts[0],
                    value: new web3.BigNumber(web3.toWei(0.001, 'ether'))
                })
                .then(results => {
