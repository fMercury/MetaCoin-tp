...
contract('MTG', accounts => {
    ...
    describe('mints', () => {
        it('works when user mints', async function() {
            let tx = await contractInstance.mint('ipfsHash', web3.toWei(0.001, 'ether'), { from: owner });
    ...
    describe('buy card', () => {
        it('buys card', async function() {
            ...
            let tx2 = await contractInstance.buyCard(tokenId, { from: accounts[1], value: web3.toWei(0.001, 'ether') });
    ...
    describe('tokensOf', () => {
        it('can get tokens of', async function() {
            await contractInstance.mint('ipfsHash', web3.toWei(0.001, 'ether'), { from: owner });
            await contractInstance.mint('ipfsHash2', web3.toWei(0.001, 'ether'), { from: owner });
            await contractInstance.mint('ipfsHash3', web3.toWei(0.001, 'ether'), { from: owner });
